#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
#fn.runEwE-----------------------------------------------------------------------------------------
fn.runEwE <-  function(dir.cmdfile,do.obj=1){
...
#   #testvals = 10
#   vulmat=vuls; testvals = c(minvul,maxvul)
#   
#   #reshape vulnerability matrix
#   longvuls = reshape2::melt(vulmat, id.vars = 1) ## Melt to long
#   names(longvuls)[2:3] = c('pred','baseval')   ## Rename columns
#   length(unique(longvuls$prey))
#   longvuls$grp.pred = rep(1:n_consumers,each=nrow(vulmat))
#   longvuls$grp.prey = rep(1:nrow(vulmat),n_consumers)
#   
#   #replicate for each testval
#   longvuls = do.call("rbind", replicate(length(testvals),longvuls, simplify = FALSE))
#   longvuls$testval = rep(testvals,each=n_consumers*n_prey)
#   longvuls = longvuls[complete.cases(longvuls),]  
#   longvuls = subset(longvuls, longvuls$baseval != longvuls$testval) ## We can remove values already at the min or max of range (i.e., if the baseval is already 1.05 or 10,000)
#   
#   #create command file tagline
#   longvuls$test.par = 'pred_prey'
#   param = "<ECOSIM_VULNERABILITIES_BY_PREY_PRED>"
#   longvuls$tag = paste0(param,",",paste(longvuls$grp.prey,longvuls$grp.pred,longvuls$testval),",System.Single[]")
#   
#   #Order by pred, then prey, then test direction
#   longvuls = longvuls[order(longvuls$grp.pred, longvuls$grp.prey, longvuls$testval), ] 
#   return(longvuls)
#   
# }
...
fn.pred_prey_testvals = function(longvuls, maxvul=100, is.maxvul.mult=T, minvul=1.01){
  
  #maxvul = 10000
  #minvul = 1.01
  if(is.maxvul.mult) longvuls$testval = longvuls$baseval*maxvul
  if(!is.maxvul.mult) longvuls$testval = maxvul
  longvuls$dir.test = "Higher"
...
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  
#fn.envpref_testvals-----
fn.envpref_testvals = function(response_vals){
  #<ECOSPACE_ENVIRONMENTAL_RESPONSE_INDEXED>(respfxn_number),shape_type par1 par2 par3 par4,Indexed.Single[]
  if(response_vals$shape_type=='trapezoid'){
    response_vals$tag = paste0("<ECOSPACE_ENVIRONMENTAL_RESPONSE_INDEXED>(",response_vals$respfxn_number,"), ",response_vals$shape_type," ",paste(response_vals$par1,response_vals$par2,response_vals$par3,response_vals$par4),", Indexed.Single[]")
  }
  return(response_vals$tag)
}
